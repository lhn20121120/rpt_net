...
<script type="text/javascript">
	...
	function turntoPage1(currentPage,pageCount){
	 ...
	 $(document).ready(function(){
		...
		$("#fm").submit(function(){
			...
			//alert($("input[name=cks]:checked").length);
	...
	*多任务报文导出 。
	*/
	function _allBbscURL(url){
		//alert("多任务导出报文");	
		var list = $("input[name=cks]:checked");
		var count=$("input[name=cks]:checked").length;
		getId("workTaskTerm").value="";
		getId("workTaskOrgId").value="";
		if(count==0){
			alert("请先选择报表！");
			return;
		}
		...
		var urlParam = "";
		//alert(getId("templateIds").value);
		getId("templateIds").value="";
		for(var i=0;i<_params.length;i++){			
			...
			if(ddata!=undefined && ddata!=""){
				...
				if(ddata.indexOf("#")>-1){
					...
					if(p!=undefined){
						...
						//alert(p[2]);
						getId("workTaskOrgId").value+=(getId("workTaskOrgId").value==""?"":",,")+p[5]+"@@"+p[0];
						//alert(p[0]);
			...
			urlParam +=( urlParam==""?"":"@@")+ddata.replace(/#/g,";;");
		}
		var taskTerm = getId("workTaskTerm").value;
		if(taskTerm.indexOf(",,")>-1){
			var count = 1;
			var terms = taskTerm.split(",,");
			var str = terms[0];
			...
			if(count>1){
				alert("系统仅支持导出同一期任务所有的报表!");
				return;
			}
		}
			//alert(urlParam);
			getId("urlParam").value=urlParam;
	...
	*多任务报文导出 。
	*/
	function _allBbscURL_to_BW(url){
		//alert("多任务导出报文");	
		var list = $("input[name=cks]:checked");
		var count=$("input[name=cks]:checked").length;
		getId("workTaskTerm").value="";
	...
	function goSubmit(url){
		var repForm = getId("repForm");
		getId("parOrgId").value = getId("orgId").value;
		repForm.action=url;
		getId("styleFlg").value="new";
		repForm.submit();
...
<table width="100%" height="20px" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
		<div class="tableinfo" ><span style="font-size:12px;"><img src="<%=request.getContextPath() %>/images/icon01.gif" alt="列表" /></span><b style="font-size:12px;">当前位置 >>报表处理 >>
		<s:if test="pendingTaskQueryConditions.busiLine==@com.fitech.model.worktask.common.WorkTaskConfig@BUSI_LINE_YJTX ||
		pendingTaskQueryConditions.busiLine==@com.fitech.model.worktask.common.WorkTaskConfig@BUSI_LINE_QTTX">
   			报表输出
   		</s:if>
   		<s:else>
   			报文生成
...
<form action="" id="repForm" target="rptFrame" method="post">
	...
	<input type="hidden" id="parOrgId" name="parOrgId"/>
	<input type="hidden" id="styleFlg" name="styleFlg"/>
	<input type="hidden" id="urlParam" name="urlParam"/>
	<input type="hidden" id="report" value="<s:property value="pendingTaskQueryConditions.busiLine"/>" name="report"/>
...
<form action="<%=request.getContextPath() %>/pendingTaskAction!findBWSCTasks.action" id="shForm" method="post">
		...
		<tr>
			<td width="15%"  align="center" >
				...
				<!-- 任务列表 <select name="pendingTaskQueryConditions.freqId" style="width: 100px;vertical-align: middle">
			   	...
			   	任务列表
			   	<s:select list="workTaskInfos" onchange="search()" headerKey="-1" headerValue="---全部---" name="pendingTaskQueryConditions.taskId" listKey="taskId" listValue="taskName" cssClass="selectClass"></s:select>
            ...
            <td width="18%" align="center" valign="middle">
            		...
            		机构  <input type="text"  id="orgName" onclick="mousefocus()" onblur="mouseblur()" style="width:150px;cursor:hand" size="25" name="pendingTaskQueryConditions.orgName" value="<s:property value="pendingTaskQueryConditions.orgName"/>" />
            </td>
            <td width="15%" align="center" valign="middle">
            	期数  <input width="100px"  id="datepicker_2"  readonly="readonly" onmouseout="hiddenDate1()" onmousemove="showDate1()" style="cursor: hand;height: 18px;vertical-align: middle" value="<s:property value="pendingTaskQueryConditions.taskTerm"/>" name="pendingTaskQueryConditions.taskTerm" onchange="search(),checkStartAndEndTime(this)"/> 
            ...
            <td width="15%" align="center" valign="middle" style="display:none;">
            	任务状态
            	<select onchange="search()" name="pendingTaskQueryConditions.nodeFlag" style="width: 100px;vertical-align: middle">
...
<%--					<option <s:if test="pendingTaskQueryConditions.nodeFlag==@com.fitech.model.worktask.common.WorkTaskConfig@NODE_FLAG_REWA">selected="selected"</s:if> value="<s:property value="@com.fitech.model.worktask.common.WorkTaskConfig@NODE_FLAG_REWA"/>">退回等待</option>--%>
            ...
            <td width="14%" align="center" valign="middle">
            	业务条线
            	<select disabled="disabled" onchange="search()" name="pendingTaskQueryConditions.busiLine" style="width: 100px;vertical-align: middle">
...
<form id="fm" action="<%=request.getContextPath() %>/pendingTaskAction!turnToRep.action" method="post">
<table id="tble" width="100%" border="0" cellspacing="0" cellpadding="0"  class="ntable" align="center">
  ...
  <tr >
   		<td colspan="10" align="left" style="height:35px;"><%--
   		<input type="submit" class="searchbtn" value="提交" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   		<input type="button" onclick="_backWorkTask()" class="searchbtn" value="退回" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   		--%>
   		<s:if test="pendingTaskQueryConditions.busiLine==@com.fitech.model.worktask.common.WorkTaskConfig@BUSI_LINE_RHTX">
   		<input type="button" onclick="_allBbscURL_to_BW('/rpt_net/exportRhAFReport.do')" class="searchbtn" value="导出报文 " />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   		</s:if>
   		<input type="button" onclick="_allBbscURL('/rpt_net/exportAFReport.do')" class="searchbtn" value="导出报表 " />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   		
   		<!-- <input type="submit" class="searchbtn" value="返回" /> -->
   		</td>
  </tr>
  <tr>
    ...
    <td width="12%" bgcolor="#FFF0D0" align="center"><strong>报送截止日期</strong></td>
    <td width="15%" bgcolor="#FFF0D0" align="center"><strong>报送阶段</strong></td>
    <td width="10%" bgcolor="#FFF0D0" align="center"><strong>状态</strong></td>
  ...
  <s:if test="pageResults!=null && pageResults.results.size()!=0">
  		<s:iterator value="pageResults.results" status="i">
  			<tr id="th<s:property value='#i.index'/>">
				...
				<input type='checkbox' name='cks' value='<s:property value="taskMoniId"/>,<s:property value="nodeId"/>,<s:property value="orgId"/>,<s:property value="busiLine"/>,<s:property value="lateRepDate"/>'/>
			  	</td>
				...
				<s:property value="taskName"/>
			  	...
			  	<td align="center"><s:date name="lateRepDate" format="yyyy-MM-dd"/></td>
			  	<td align="center" ><s:property value="taskNodeName"/></td>
			  <!--	<td align="center" ><font onmouseover="taskMove(<s:property value="taskMoniId"/>,this)" ><s:property value="taskNodeName"/></font></td>--> 
			  	<td align="center">
...
　</fieldset>
<!-- <div id="dv" onmousemove="move(this)" onmouseout="out(this)">
<div id="d1" ><img src="<%=request.getContextPath() %>/images/u0_normal1.png"  onclick="changeImg()" style="cursor:hand;"/></div>
	<div style="width:100%;" ><img src="<%=request.getContextPath() %>/images/bg2.jpg"/></div>
	<div id="d2" ><img src="<%=request.getContextPath() %>/images/u0_normal2.png" onclick="changeImg()" style="cursor:hand;"/></div>
	</div> -->
<div id="d1" ><img src="<%=request.getContextPath() %>/images/task_up.gif"  onclick="changeImg()" style="cursor:hand;"/></div>	
	...
	<tr>
   		<td colspan="7" height="5%" bgcolor="#FFF0D0"><b><font id="f1">任务处理列表</font>>><font id="f2">
