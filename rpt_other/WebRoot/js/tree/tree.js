...
function TreeFmt( fmt, tree ){
    this.init=function( fmt, tree ){
        ...
        this.ident=fmt[8];
        if (!tree.ver3)
            this.back=new ListTreeBack(this.left, this.top, fmt[9], 'cls'+tree.name+'_back');
        this.nst=fmt[10];
        this.nstl=fmt[11];
        this.so=fmt[12];
        this.pg=fmt[13][0];
        ...
        this.exp=fmt[14];
        this.expimg=fmt[15];
        this.expimgsize=fmt[16];this.cook=fmt[17];
        this.rel=fmt[18];
        this.rels=fmt[19];
        this.resize=fmt[20];
        this.sel=fmt[21];
        this.selC=fmt[22];
        this.selClass=fmt[22]?fmt[22][2]:'';
        this.nnOnMOver=fmt[24];
...
function ListTree( name, nodes, format ,checkName,clickEnv,enableSub,enableParent){
    this.checkIndex = 0;
    this.name=name;
    ...
    this.id='CTree_'+name;
    eval(this.id+'=this');
    if (und(window.CTrees))
        window.CTrees=new Array();
    ...
    this.rootNode=new ListTreeNode(null, "", "", "", null,"0","checked");
    ...
    this.nbn=function(nm){for (var i=0; i<this.Nodes.length; i++)
        if (this.Nodes[i].text == nm)
            return this.Nodes[i];
        return null;
    };
    this.nodeByName=this.nbn;
    this.nodeByID=function(id){for(var i=0;i<this.Nodes.length;i++)
    ...
    this.rebuildTree=function(){
        ...
        for (var i=0;i < nodes.length; i++){
            ...
            s +=nodes[i].init(this.checkName,i,this.clickEnv);
        }
        if (this.fmt.rel)
            if (this.bw.ns4){
                ...
                s='<div id="cls'+this.name+'_back" style="position:absoluted;left:0px;'+bgc+'top:0px;width:'+this.fmt.rels[0]+'px;height:'+this.fmt.rels[1]+'px;">' + s +'</div>';
            }
        document.write(s);
    ...
    this.getImg = function(node){
        if (this.ns4) {
            if (this.fmt.showF)
                node.nf=node.el.document.images[node.id()+"nf"];
            if (this.fmt.showB)
                node.nb=node.el.document.images[node.id()+"nb"];
        }else{
            if (this.fmt.showB)
                node.nb=node.el.all? node.el.all[node.id()+"nb"] : document.getElementById(node.id()+"nb");
            if (this.fmt.showF)
                node.nf=node.el.all? node.el.all[node.id()+"nf"] : document.getElementById(node.id()+"nf");
        }
    };
    this.draw=function(){
        if ( this.ver3 )
    ...
    this.expandNodeCheck=function( index, nd){
        if ( this.ver3 ) return;
        var node=this.Nodes[index];
        var state = document.getElementById(this.checkName+"-"+index).checked;
        if(this.enableSub){
            this.setSubCheck(node,state);
        }
        if(this.enableParent){
            this.setParentCheck(node,state);
        }
        if (!nd && this.fmt.cook) this.saveState();
        if (!nd){
            this.draw();
    ...
    this.setSubCheck=function(node,state){
        if (!und(node) && node.hasChildren()){
            for (var i=0; i < node.children.length; i++){
                document.getElementById(this.checkName+"-"+node.children[i].checkboxIndex).checked = state;
                this.setSubCheck(node.children[i],state);
            }
        }
    }
    this.setParentCheck=function(node,state){
        if(state){
            var pNode=node.parentNode ? node.parentNode : null;
            if (pNode){
                document.getElementById(this.checkName+"-"+pNode.checkboxIndex).checked = state;
                this.setParentCheck(pNode,state);
            }
        }
    }
    ...
    this.readOne=function( par, arr , tree){
        ...
        var node=this.addNode(new ListTreeNode(this, par, arr[i], arr[i+1]==null? "": arr[i+1], arr[i+2]==null? "": arr[i+2],arr[3],arr[4]));
        node.nodeID=nid;
        var itemCount = 5;
        if (!und(arr[i+itemCount]) && !und(arr[i+itemCount]["format"])){
            node.f=arr[i+itemCount]["format"];
            node.expanded=und(node.f.expanded)?false:node.f.expanded;  i++;
        }
        while (!und(arr[i+itemCount])){
            par=node;
            this.readOne(par, arr[i+itemCount]);  i++;
        }
    };
    ...
    this.saveState=function(){
        var state="";
        for (var i=0; i < this.Nodes.length; i++)
            state +=this.Nodes[i].expanded ? '1' : '0'; /* Uncomment this to keep cookies during month var d = new Date(); d.setMonth(d.getMonth()+15); document.cookie=this.name+'State='+state+'; path=/'+'; expires='+d.toGMTString(); */
        if (this.selectedNode!=null)
            document.cookie=this.name+'Selected='+this.selectedNode.index; document.cookie=this.name+'State='+state+'; path=/';
    };
...
function ListTreeNode( treeView, parentNode , text, url, target,nodeValue,checkStatus){
    ...
    this.initImages=function (){
        ...
        if (this.treeView.fmt.exp){
            ...
            var img=ei[2];
            this.cimg=this.hasChildren() ? ei[0] : img;  this.eimg=this.hasChildren() ? ei[1] : img;
            var ii=0;
    ...
    this.init=function(checkName,indexId,onclickEvent){
        var bw=this.treeView.bw;
        this.checkboxIndex = indexId;
        return !this.treeView.ver3?this.treeView.ns4?'<layer id="'+this.id()+'d" z-index="'+this.index+10+'" display="none">'+this.getContent(checkName,indexId,onclickEvent)+'</layer>': '<div id="'+this.id()+'d" style="display:none;'+(bw.opera&&(bw.nver!=6)?'width:1px;':'')+'z-index:'+this.index+10+';">'+this.getContent(checkName,indexId,onclickEvent)+'</div>': this.getContent(checkName,indexId,onclickEvent);
    };
    this.getH=function(){
        bw=this.treeView.bw;
    ...
    this.getContent=function(checkName,indexId,clickEvent){
        ...
        if ( this.treeView.fmt.showB || this.treeView.fmt.exp)
            s +=this.treeView.fmt.exp ? buttonSquare(this) : (this.hasChildren() ? buttonSquare(this) : blankSquares(this, this.treeView.fmt.Ew));
        ...
        if(clickEvent && clickEvent.length>0){
            clickEvent = clickEvent.replace(/#KEY#/g,this.nodeValue);
            clickEvent = clickEvent.replace(/#CAPTION#/g,this.text);
            u = "javascript:"+clickEvent+""+exp;
        }
        ...
        var t = '<a'+onm+' id="'+this.id()+'an" class="'+this.treeView.fmt.nstyle(ll)+'" href="'+u+'" '+targ+'>'+this.text+'</a>';
        s +=this.treeView.ns4?'<td nowrap=\"1\"><ilayer id="'+this.id()+'a">'+t+'</ilayer></td></tr></table>' : '<td nowrap=\"1\"><div id="'+this.id()+'a">';
        if(checkName && checkName.length>0){
            s += '<input type="checkbox" name="'+checkName+'" id="'+checkName+'-'+indexId+'"   value="'+this.nodeValue+'" '+this.checkStatus+' onclick="window.CTrees[\''+this.treeView.name+'\'].expandNodeCheck('+this.index+',1)">';
        }
        s += t+'</div></td></tr></table>';
    ...
    this.moveTo=function( x, y ){
        var bw=this.treeView.bw; var px=+bw.dom&&!bw.operaOld?'px':'';
        if (this.treeView.ns4)  this.el.moveTo(x,y);
        else {  this.el.style.left=x+px;  this.el.style.top=y+px; }
    };
    this.show=function(sh){
        if (this.visible==sh)
            return;
        this.visible=sh;
        var vis=this.treeView.ns4 ? (sh ? 'show': 'hide') : (sh ? 'block': 'none');
    ...
    this.setBGColor=function (color){
        ...
        if (this.treeView.bw.ns4)el.bgColor=color;
        else if (el.style)
...
function ListTreeBack( aleft, atop, color, name ){
    this.bw=new bw_check();
    this.ver3=this.bw.ver3;
    this.ns4=this.bw.ns4;
    this.left=aleft;
...
function RedrawAllTrees(){
    for (var tree in window.CTrees)
        window.CTrees[tree].draw()
}
...
function resizeHandler() {
    ...
    var reload=window.innerWidth != window.origWidth || window.innerHeight != window.origHeight; window.origWidth=window.innerWidth;window.origHeight=window.innerHeight;
