...
<html:html lang="true">
  <head>
    ...
    <script language="javascript">		
			...
			var orgIds="";
				...
				window.location="<%=request.getContextPath()%>/template/mod/preEditBSFW.do?childRepId="+formObj.childRepId.value+"&versionId="+formObj.versionId.value+"&orgId="+formObj.upOrgId.value
				             +"&orgIds="+orgIds;
			}			
  ...
  <body>
    ...
    <html:form action="/template/add/insertBSFW" method="post" styleId="frmSel">
	    ...
	    <TABLE cellSpacing="1" cellPadding="4" width="96%" border="0" align="center" class="tbcolor">
			...
			<logic:present name="isNull" scope="request">
				<tr colspan="8">
			  	 ...
			  	 <tr>				
					   <td align="center">
					       <input type="button" property="back" value="返  回" class="input-button" onclick="window.location.assign('<%=request.getContextPath()%>/template/viewMChildReport.do')"/>		  
					    </td>					  
			    	</tr>    
			 </logic:present>
	...
	function getAllCheckedId(){
				...
				window.location="<%=request.getContextPath()%>/template/add/insertBSFW.do?orgIds="+checkId+"&childRepId="+'<%=childRepId%>'+"&versionId="+'<%=versionId%>';
				}
	}		
