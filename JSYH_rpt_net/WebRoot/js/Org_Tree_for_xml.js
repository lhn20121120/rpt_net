...
function dhtmlXTreeItemObject(itemId, itemText,orgInfo, parentObject, treeObject, actionHandler, mode) {  
    this.htmlNode = "";
    this.acolor = "";
    this.scolor = "";
    this.tr = 0;
...
dhtmlXTreeObject.prototype._attachChildNode = function (parentObject, itemId, itemText,orgInfo, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode) {
    if (beforeNode) {
    ...
    Nodes[Count] = new dhtmlXTreeItemObject(itemId, itemText,orgInfo, parentObject, this, itemActionHandler, 1);
...
dhtmlXTreeObject.prototype.insertNewItem = function (parentId, itemId, itemText,orgInfo, itemActionHandler, image1, image2, image3, optionStr, childs) {
    var parentObject = this._globalIdStorageFind(parentId);
    if (!parentObject) {   
        return (-1);
    }
    return this._attachChildNode(parentObject, itemId, itemText,orgInfo, itemActionHandler, image1, image2, image3, optionStr, childs);
};
dhtmlXTreeObject.prototype._parseXMLTree = function (dhtmlObject, node, parentId, level) {
    ...
    for (var i = 0; i < node.childNodes.length; i++) {
        if ((node.childNodes[i].nodeType == 1) && (node.childNodes[i].tagName == "item")) {
            ...
            var info = node.childNodes[i].getAttribute("orgInfo");
            ...
            temp.XMLload = 1;
            dhtmlObject.insertNewItem(parentId, cId, name,info, 0, im0, im1, im2, zST, chd);
...
dhtmlXTreeObject.prototype._correctPlus = function (itemObject) {
    ...
    //      itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[0].childNodes[0].className="imgStyle";
    }
};
...
dhtmlXTreeObject.prototype._createItem = function (acheck, itemObject, mode) {
    ...
    var infos=itemObject.info;
  
	itemObject.span.title =infos;
    td2.style.width = "100%";
...
dhtmlXTreeObject.prototype.getOrgInfo = function (itemId) {
    var temp = this._globalIdStorageFind(itemId);
    if (!temp) {
        return 0;
    }
...
dhtmlXTreeObject.prototype.setOrgInfo = function (itemId, newLabel) {
    var temp = this._globalIdStorageFind(itemId);
    if (!temp) {
        return 0;
    }
...
dhtmlXTreeObject.prototype._getAllChecked = function (htmlNode, list, mode) {
    ...
    if (((mode) && (htmlNode.checkstate == 1 || htmlNode.checkstate == 2)) || ((!mode) && (htmlNode.checkstate > 0))) {
        if (list) {
            list += ",!," + htmlNode.id;
        } else {
            list = htmlNode.id;
...
dhtmlXTreeObject.prototype.insertNewNext = function (parentItemId, itemId, itemName, itemActionHandler, image1, image2, image3, optionStr, childs) {
    ...
    this._attachChildNode(0, itemId, itemName,orgInfo, itemActionHandler, image1, image2, image3, optionStr, childs, sNode);
};
