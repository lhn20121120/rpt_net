 ...
 */
function goback() {
	//window.location = "<%=request.getContextPath()%>/afReportAdd.do";
	history.back();
	}			
	...
	<html:form action="/template/mod/updateBSFW.do" method="post" styleId="frmSel">
		...
		<TABLE cellSpacing="1" cellPadding="4" width="96%" border="0" align="center" class="tbcolor">
			<TR class="tbcolor1">
				<th align="center" id="list" height="30" colspan="2">
					<span style="FONT-SIZE: 11pt"> <logic:present name="ReportName" scope="request">
						...
						<input type="hidden" value='<bean:write name="ReportName" />' id="reportNamehidden">
				</th>
			</TR>
			...
			<logic:notPresent name="isNull" scope="request">
				...
				<tr>
					<td align="center">
						<input type="button" value="下一步" class="input-button" onclick="getAllCheckedId()">
						<%--
						&nbsp;
						<input type="button" value="返  回" class="input-button" onclick="goback();" />
					--%></td>
...
function getAllCheckedId() {
	var reportName = document.getElementById("reportNamehidden").value;
	//alert(reportName);
	...
	//window.location="<%=request.getContextPath()%>/gznx/hzformula.do?orgIds=" + checkId
			    ///  + "&childRepId=<%=childRepId%>&versionId=<%=versionId%>&reportName="+reportName+"&opration=next";                                                           
	}
}
