...
<html:html locale="true">
	...
	function goback()
	{
			...
			window.location="<%=request.getContextPath()%>/viewAFTemplateDetail.do?templateId=<%=request.getAttribute("templateId")%>&versionId=<%=request.getAttribute("versionId")%>&bak2=2";
	}	
	function _query(){
		var frmHZGS=document.forms['frmHZGS'];
		var templateId=document.getElementById("templateId").value;
		var orgName=document.getElementById("orgName").value;
		frmHZGS.action="<%=request.getContextPath()%>/gznx/EditHZGS.do?templateId="+templateId+"&orgName="+orgName+"&opration=edit&versionId=<%=versionId%>";
//		alert(frmHZGS.action);
	...
	<body style="TEXT-ALIGN: center">
			...
			<html:form action="/gznx/hzformula" method="POST" styleId="frmHZGS">
				...
				<table border="0" width="98%">	
					<TR>
						...
						<td width="10%" align="left">
							<input type="button" value="查  询" class="input-button" onclick="_query()"/>
						</td>
									...
									<logic:present name="Records" scope="request">
											<logic:iterate id="item" name="Records">
												<TR bgcolor="#FFFFFF" onmouseover="changeToLavender(this)" onmouseout="changeToWhite(this)">
													...
													--%>
													<TD align="center">
														<a href="<%=request.getContextPath()%>/gznx/HZRule.do?templateId=<bean:write  name="item" property="templateId" />&versionId=<bean:write  name="item" property="versionId" />&reportName=<bean:write  name="item" property="reportName" />&orgId=<bean:write name="item" property="orgId"/>&orgName=<bean:write name="item" property="orgName"/>&optype=mod">汇总公式设定</a>
													</TD>
												</TR>
											</logic:iterate>
									</logic:present>
