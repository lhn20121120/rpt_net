...
<%
	...
	String excelPath = (Config.WEBROOTPATH+com.fitech.net.config.Config.REPORT_NAME+File.separator+com.fitech.net.config.Config.TEMPLATE_NAME);
		...
		function showEditOnline(obj){
			...
			if(mes!=""){
				try{
					...
					$.post(url,function(cellDesc){
						if(cellDesc!=null && cellDesc!=""){
							var cellNameArr=cellDesc.split("@");
							//迭代所有的单元格数据
							$.each(cellNameArr, function(i, n){
							  	colNameAndRowName=n;
							  	var descArr=colNameAndRowName.split("_");
							    var cell=descArr[0];//获得单元格名称
								var colName = descArr[1];//获得列名
								var rowName = descArr[2];//获得行名
								...
								var newTD_2 = $("<td>"+cell+"<input type='hidden' name='templateIdAndVersionIdAndCellName' value='"+templateId+"_"+versionId+"_"+cell+"' /></td>");
...
<body class="easyui-layout">
	...
	<div region="north" title="查询条件" split="true" style="height:90px;padding:8px;background:#efefef;">
		...
		<table style="width: 100%;height: 100%;text-align: center;">
			<tr>
				...
				<td style="width:20%;">
					<span style="vertical-align: bottom;">开始时间：</span>
					<input id="startDate" name="startDate" value ="<%=date%>" class="easyui-datebox"  editable="false"></input>
				</td>
				<td>
					<span style="vertical-align: bottom;">结束时间：</span>
					<input id="endDate" name="endDate" value ="<%=date%>" class="easyui-datebox"  editable="false">
				</td>
	...
	<div region="east" iconCls="icon-reload" title="机构" split="true" style="width:180px;">
		<input type="checkbox" id="chkCascade" name="chkCascade" checked="checked" onclick="cascadeAction()">机构是否级联选择
			...
			<select id="leftList" name="leftList" multiple="multiple" size="16" style="width: 100%;height:100%" ondblclick="showEditOnline(this)">
				<%
					if(request.getAttribute("reportList")!=null && !request.getAttribute("reportList").equals("")){
						List<MChildReportForm> reportList = (List<MChildReportForm>)request.getAttribute("reportList");
						if(reportList!=null && reportList.size()>0){
							for(MChildReportForm m : reportList){
								String repName = m.getReportName();
								String versionId = m.getVersionId();
								String reportId = m.getChildRepId();
								String relPath = reportId+"_"+versionId;
				%>
					<option value="<%=relPath %>"><%="["+reportId+"_"+versionId+"]"+repName %> </option>
				<%	
							}
						}
					}
	...
	<div region="center" title="单元格" style="overflow:hidden;">
			<div title="Tab1" id="shuomingDIV" style="padding:20px;overflow:hidden;display: block" > 
				...
				<div id="findLastDIV" style="overflow: auto;display: none">
					<form action="searchDataAction.do" id="searchForm" method="post">
						<table style='width:100%;' id="dataTable" align='center' align='center' cellpadding='0' cellspacing='0' >
							...
							<tr align='center' onmouseover="this.backgroundColor='#EEEEFF'" onmouseout="this.backgroundColor='white'" <%--onclick="checkedData(this)"--%>>
								<th style="width: 10px"><input type="checkbox" onclick="chkAction($(this).attr('checked'))"></th>
								<th>报表名称</th>
								<th>单元格名称</th>
								<th>横向</th>
								<th>纵向</th>
