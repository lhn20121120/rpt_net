	...
	function showTree1(){
		...
		function org_validate_Select(){
			...
			if(repInIds==""){
		  	  	...
		  	  	if(confirm("您确定要进行总分校验吗?\n")==true){
					
						var validateURL = "<%=request.getContextPath()%>/collectValidate.do?repInIds="+repInIds;
		...
		function org_validate_all(){
		 	var qry="templateId=" + document.getElementById("templateId").value + 
		  	  	...
		  	  	if(confirm("总分校验全部需要较长时间，您确定要进行总分校验全部操作吗?")==true){

						var validateURL = "<%=request.getContextPath()%>/collectValidate.do?"+qry;
		...
		var result = request.responseXML.getElementsByTagName('result')[0].firstChild.data;
			if(result == 'true'){
				...
				alert('存在无上报数据的报表记录！请确认！');	   	     
			}else{
				if(confirm('校验未通过！是否需要查看校验信息?')){
					window.open("<%=request.getContextPath()%>/report/viewOrgValidateInfo.do?repInIds=" + repInIds,'总分校验结果','scrollbars=yes,height=600,width=500,status=yes,toolbar=no,menubar=no,location=no,resizable=yes');
				}
			} 
			//selectReport();
			document.getElementById("frm").submit();
	    ...
	    //查看总分校验结果
	    function view_ZFResult(repInIds){
	    	window.open("<%=request.getContextPath()%>/report/viewOrgValidateInfo.do?repInIds=" + repInIds,'总分校验结果','scrollbars=yes,height=600,width=500,status=yes,toolbar=no,menubar=no,location=no,resizable=yes');
	    }
			...
			<html:form action="/dateQuary/saveAllCheckRepNXRecheck" method="post" styleId="frm1">
				<tr>
					<td>
						...
						&nbsp;&nbsp;∷&nbsp;&nbsp;
						<input type="button" value="总分校验选中" class="input-button" onclick="org_validate_Select()">
						&nbsp;
						<input type="button" value="总分校验全部" class="input-button" onclick="org_validate_all()">
						&nbsp;&nbsp;∷&nbsp;&nbsp;
						...
						<logic:present name="form" scope="request">
							<logic:iterate id="viewReportIn" name="form">
								...
								<TR bgcolor="#FFFFFF">
									...
									<TD align="center"><%-- 2013-05-31使用AbmormityChangeFlag作为总分校验在前台显示的标志位 --%>
										<logic:present name="viewReportIn" property="abmormityChangeFlag">
											<logic:equal name="viewReportIn" property="abmormityChangeFlag" value="1">
												校验通过
											</logic:equal>
											<logic:equal name="viewReportIn" property="abmormityChangeFlag" value="-1">
												<a href="javascript:view_ZFResult(<bean:write name='viewReportIn' property='repInId'/>)"><font color="red">校验不通过</font> </a>
											</logic:equal>
										</logic:present>
										<logic:notPresent name="viewReportIn" property="abmormityChangeFlag">
											未校验
										</logic:notPresent>
									</TD>
									<%--
