...
<head>
	...
	<script language="javascript" src="<%=Config.WEBROOTULR%>/script/qyts//scripts/jquery-1.3.2.min.js"></script>
		...
		function _collect(templateId,versionId,year,term,day,repFreqId,curId,orgId,donum){
			...
			if(donum==0){
				...
				if(confirm("确认要汇总吗？")){
					...
					var param=templateId+","+versionId+","+orgId+","+curId+","+repFreqId+","+donum+","+year+","+term+","+day+"#";
					document.getElementById('iterFlag').value="0";
					document.getElementById('select_data_collect_id0').value=param;
					
					var searchOrgId = document.getElementById("frm").orgId.value;
		   			var searchOrgName = document.getElementById("frm").orgName.value;
		   			var searchRepFreqId = document.getElementById("frm").repFreqId.value;
		   			var searchSupplementFlag = document.getElementById("frm").supplementFlag.value;
		   			document.getElementById("collect_orgId").value = searchOrgId;
		   			document.getElementById("collect_orgName").value = searchOrgName;
		   			document.getElementById("collect_repFreqId").value = searchRepFreqId;
		   			document.getElementById("collect_supplementFlag").value = searchSupplementFlag;
		...
		function _selectAll(){
			if(document.getElementById('data_collectAll').checked==false){
				_cancelAll()
		  		...
		  		//}
		  		$("input[name='data_collect']").each(function () {
					this.checked = true;
				});
			}
	  	}
...
<body<%-- onload="initMethod(this)"--%>>
	...
	<label id="prodress1" >
		...
		<form name="form2" action="<%=request.getContextPath()%>/collectReportNX.do"
			onsubmit="return _collect_select(this)" method="post" style="margin: 0,0,-10,0;">
			<logic:present name="Records" scope="request">
				<table width="99%" border="0" cellpadding="4" cellspacing="1">
					...
					<input type="hidden" id="collect_repFreqId" name ="repFreqId"/>
					<input type="hidden" id="collect_supplementFlag" name ="supplementFlag"/>
		...
		<logic:present name="add_old_collect">
			...
			<logic:present name="Records" scope="request">
				<logic:iterate id="viewReportIn" name="Records" indexId="index">
					...
					<tr bgcolor="#FFFFFF">
						<td align="center">
						<logic:greaterThan value="0" name="viewReportIn" property='donum' >
