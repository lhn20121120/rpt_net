...
/**
 * Displays a MSupvOrg. We'll first try to look for a MSupvOrg
 ...
 */
public final class ViewMSupvOrgAction extends Action {
   ...
   public ActionForward perform(
      ...
      MSupvOrgForm MSupvOrgForm = (MSupvOrgForm)request.getAttribute("MSupvOrg");
      if( MSupvOrgForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            MSupvOrgForm = new MSupvOrgForm();
            // Now set the form members with request values 
            RequestUtils.populate(MSupvOrgForm, request);

            MSupvOrgForm = com.cbrc.org.adapter.StrutsMSupvOrgDelegate.edit(MSupvOrgForm);
      ...
      // Make a map that can be used to identify this MSupvOrg in any links
      HashMap urlParams = new HashMap();
  urlParams.put("orgId",MSupvOrgForm.getOrgId());
