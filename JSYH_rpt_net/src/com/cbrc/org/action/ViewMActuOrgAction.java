...
/**
 * Displays a MActuOrg. We'll first try to look for a MActuOrg
 ...
 */
public final class ViewMActuOrgAction extends Action {
   ...
   public ActionForward perform(
      ...
      MActuOrgForm MActuOrgForm = (MActuOrgForm)request.getAttribute("MActuOrg");
      if( MActuOrgForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            MActuOrgForm = new MActuOrgForm();
            // Now set the form members with request values 
            RequestUtils.populate(MActuOrgForm, request);

            MActuOrgForm = com.cbrc.org.adapter.StrutsMActuOrgDelegate.edit(MActuOrgForm);
      ...
      // Make a map that can be used to identify this MActuOrg in any links
      HashMap urlParams = new HashMap();
  urlParams.put("actuOrgTypeId",MActuOrgForm.getActuOrgTypeId());
