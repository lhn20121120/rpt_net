				...
				for (int i = 0; i < cellList.size(); i++)
				{
					...
					if(isAddTrace){
						...
						String henjiSQL = "select "+sumFormu+" from af_data_trace t " +
						  ...
						  "on r.child_rep_id=a.template_id and r.version_id=a.version_id and a.cell_name=t.cell_name " +
...
//							System.out.println(sumData);
					...
					String insertSQL = "";
					try{
						if(isAddTrace&&Config.ISADDTRACE){
   ...
   private List getDataFromExcels(String repInId) throws Exception{      
       ...
       try{
           ...
           for (int i = 0; i < size; i++){
               Cell cell = (Cell) cellList.get(i);
               //调试用
//               if (cell.getColId().equals("T") && cell.getRowId()==17)
