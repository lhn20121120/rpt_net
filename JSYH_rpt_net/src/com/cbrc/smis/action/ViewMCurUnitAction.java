 ...
 * @author 唐磊
 *
 * @ViewMCurUnitAction ；货币单位浏览数据的Action对象
 *
 */
public final class ViewMCurUnitAction extends Action {
	private static FitechException log = new FitechException(ViewMCurUnitAction.class); 
   ...
   public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
      ...
      ApartPage aPage=new ApartPage();
		...
		{
	   		//取得记录总数
	   		recordCount = StrutsMCurUnitDelegate.getRecordCount(mCurUnitForm);
	   		//显示分页后的记录
	   		if(recordCount > 0)
		   	    resList = StrutsMCurUnitDelegate.select(mCurUnitForm,offset,limit);  
		}
		catch (Exception e) 
		{
		...
		//把ApartPage对象存放在request范围内
	 	aPage.setTerm(this.getTerm(mCurUnitForm));
   ...
   public String getTerm(MCurUnitForm mCurUnitForm)
   {
	   String term="";
	   ...
	   {
		   term += (term.indexOf("")>=0 ? "" : "&");
		   term += "curUnit="+curUnit.toString();   
	   }
	   if(curUnitName!=null && !curUnitName.equals(""))
	   {
		   term += (term.indexOf("")>=0 ? "" : "&");
		   term += "curUnitName="+curUnitName.toString();   
	   }
	   return term;
   }
   }