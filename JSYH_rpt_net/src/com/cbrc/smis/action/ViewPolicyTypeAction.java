...
/**
 * Displays a policyType. We'll first try to look for a policyType
 ...
 */
public final class ViewPolicyTypeAction extends Action {
   ...
   public ActionForward execute(
      ...
      PolicyTypeForm policyTypeForm = (PolicyTypeForm)request.getAttribute("policyType");
      if( policyTypeForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            policyTypeForm = new PolicyTypeForm();
            // Now set the form members with request values 
            RequestUtils.populate(policyTypeForm, request);

            policyTypeForm = com.cbrc.smis.adapter.StrutsPolicyTypeDelegate.edit(policyTypeForm);
      ...
      // Make a map that can be used to identify this PolicyType in any links
      HashMap urlParams = new HashMap();
  urlParams.put("policyTypeId",policyTypeForm.getPolicyTypeId());
