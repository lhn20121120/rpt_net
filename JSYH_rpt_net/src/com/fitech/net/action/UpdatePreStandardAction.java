...
public final class UpdatePreStandardAction extends Action {
    private static FitechException log = new FitechException(
    		UpdatePreStandardAction.class);
  ...
  boolean result=false;
        ...
        {
        	//删除历史数据
        	result=StrutsTargetDefineWarnDelegate.delete(targetDefineWarnForm,com.fitech.net.config.Config.Target_Pre_Standard);
        	//纪录转换成list
        	if(result==false)
        	{
        		messages.add("更新失败");
    ...
    private List GetWarnList(TargetDefineWarnForm targetDefineWarnForm)
    {
    	...
    	if(arr!=null && arr.length>0){
			...
			for(int i=0;i<arr.length;i++){
				...
				temp.setType(com.fitech.net.config.Config.Target_Pre_Standard);//预警指标
