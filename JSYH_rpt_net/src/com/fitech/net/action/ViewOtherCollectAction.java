...
public class ViewOtherCollectAction  extends Action {
	private static FitechException log = new FitechException(ViewOtherCollectAction.class);
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,
        ...
        try{
        	//取得记录总数
        	recordCount = StrutsReportInDelegate.getOtherCollectRecordCount(reportInForm);
        	
        	//显示分页后的记录
            if(recordCount > 0)
                list = StrutsReportInDelegate.getOtherCollectRecords(reportInForm,offset,limit);  
        }catch(Exception ex){
       	 log.printStackTrace(ex);
            messages.add(resources.getMessage("select.otherCollect.failed"));  
        }
