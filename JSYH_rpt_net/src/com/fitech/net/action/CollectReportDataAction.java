					...
					if (isMulti != null && isMulti.equals("multi")){
						...
						for (int i = 0; i < parArr.length; i++){
							...
							if(donum.intValue() == 0){
								 ...
								 * 获取子机构Id
								 * **/
								List orgLst=VorgCollectDelegate.getRepOrg(childRepIdAll, versionIdAll, dataRangeIdAll, reportInForm.getYear(), reportInForm.getTerm(),reportInForm.getCurId(), orgId);
								String childOrgIds="";
								if(orgLst!=null && orgLst.size()>0){
									Aditing aditing=null;
						...
						/*****************************************/
						String orgId=request.getParameter("orgId");
						List orgLst=VorgCollectDelegate.getRepOrg(childRepId, versionId, dataRangeId, reportInForm.getYear(), reportInForm.getTerm(),reportInForm.getCurId(), orgId);
						String childOrgIds="";
						if(orgLst!=null && orgLst.size()>0){
							Aditing aditing=null;
							for(int i=0;i<orgLst.size();i++){
