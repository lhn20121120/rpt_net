			...
			ServletException
	{
		...
		try
		{
			...
			if (aforg.getOrgType().equals("-99")){
				resList=VorgCollectDelegate.getRepOrg(childRepId,versionId,dataRangeId,year,month,currId,orgId);
						...
						for(int j =0 ;j<childRepArr.length;j++){
							...
							if(mustOrgs!=null && mustOrgs.size()>0){
								for(int i = 0 ;i<mustOrgs.size();i++){			
									...
									/**2013-11-01 LuYueFei 因为新增了拼接汇总的流程，因此处理前台数据显示信息*/
									StrutsCollectDelegate.setJoinTemplateInfo(aditing, dataRangeId, year, month, currId, orgid);
									resList.add(aditing);
								}						
							}
						}	
