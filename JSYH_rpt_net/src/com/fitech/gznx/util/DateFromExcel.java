	...
	public String saveFromExcel(String reportId, String templateId,
		...
		try {
			...
			if(reportData!=null && reportData.size()>0 && rd!=null && rd.getDataSetMetaData()!=null){
				...
				com.fitech.gznx.util.RaqUtil.removeDataSetFromRaq(rd, templateId);
				...
				if(dataSet!=null && dataSet.getDataSetConfigCount()>0){
					...
					while(j<count){
						...
						if(df!=null && df.getName()!=null){
							temp = j;
							j++;
							if(!df.getName().trim().toLowerCase().equals("ds_org") && !df.getName().trim().startsWith(templateId.trim())){
...
//				iExcelCell.setInputProperty(input);			
			...
			for (Object[] cell:celllist) {
				...
				// 设置填报属性
				InputProperty input = new InputProperty();
				if(iExcelCell.getBackColor() == Config.DGRAY_BGCOLOR || iExcelCell.getBackColor() == Config.GRAY_BGCOLOR
						|| iExcelCell.getBackColor() == Config.LGRAY_BGCOLOR){//灰色单元格取消回填功能
					input.setWritable(false);
				}else{
					input.setWritable(true);
				}
				input.setEmptyIsNull(true);
