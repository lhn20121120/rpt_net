...
public class PreEditHZSDAction extends Action {
...
//		System.out.println("汇总设定 action ");
		...
		if (session.getAttribute("SelectedOrgIds") == null) {
			...
			List selectedOrgFromDB = StrutsTemplateOrgCollRelationDelegate.findAll(
					childRepId, versionId);
			if (selectedOrgFromDB != null && selectedOrgFromDB.size() > 0) {
				for (int i = 0; i < selectedOrgFromDB.size(); i++) {
					AfTemplateCollRep item = (AfTemplateCollRep) selectedOrgFromDB
							.get(i);
					//判断是否包含父节点
