							...
							for(int i=1;i<=rownum;i++){
								for(int j=1;j<=colnum;j++){					
									...
									if(map != null && !map.isEmpty() && map.getValue(0) != null){
										...
										if(val.indexOf("报表日期：")>-1||val.indexOf("报表日期:")>-1){
											cs.setValue("报表日期： ${report.year} 年${report.term} 月");
										}else if(val.indexOf("填报机构：")>-1||val.indexOf("填报机构:")>-1){
											cs.setValue("填报机构： ${report.orgName}");
										}else if(val.indexOf("币种：")>-1||val.indexOf("币种:")>-1){
											cs.setValue("币种： ${report.curName}");
										}
									}
								}
							}
...
//			}
		...
		private AFTemplateForm copyTemplateInfo(MChildReportForm childReportForm) {
			...
			if(childReportForm.getJoinTemplateId() !=null && !childReportForm.getJoinTemplateId().trim().equals("")){
				templateForm.setJoinTemplateId(childReportForm.getJoinTemplateId().trim());
			}
