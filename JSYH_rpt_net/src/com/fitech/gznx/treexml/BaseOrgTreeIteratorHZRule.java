...
public class BaseOrgTreeIteratorHZRule implements OrgTreeInterface {
	...
	public boolean createTreeForTagXml() {
		...
		try {
			...
			for (int i = 0; i < orgList.size(); i++) {
				...
				if(!strs[0].equals(com.fitech.gznx.common.Config.HEAD_ORG_ID)){continue;}//如果不是总行根节点，则不执行下面的操作
	...
	public void iteratorCreate(Element element) {
		...
		//2.虚拟机构机构树展示
		if(element.attributeValue("id").equals(com.fitech.gznx.common.Config.HEAD_ORG_ID)){//如果上级机构ID为根节点，则加上虚拟机构信息
			element.addAttribute("id", com.fitech.gznx.common.Config.VIRTUAL_TOPBANK);//由于已经存在id节点，因此使用addAttribute方法此时会替换掉原有的值
			iteratorCreate(element);//迭代生成所有的虚拟机构节点信息
		}
	}
