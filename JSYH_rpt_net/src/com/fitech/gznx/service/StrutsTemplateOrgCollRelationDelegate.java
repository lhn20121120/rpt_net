...
import com.fitech.gznx.po.AfTemplateCollRepId;

public class StrutsTemplateOrgCollRelationDelegate {

	private static FitechException log = new FitechException(StrutsTemplateOrgCollRelationDelegate.class);

	/**
	 * 影响对象：AfTemplateOrgRelation 根据子报表ID和版本号获取其报送范围信息列表
	 * 
	 * @param childRepId
	 *            String 子报表ID
	 * @param versionId
	...
	public static List findAll(String templateId, String versionId ) {
		...
		DBConn conn = null;
		try {
			String hql = "from AfTemplateCollRep mrr where mrr.id.templateId='" + templateId
	...
	public static boolean removeLowerOrg(String childRepId, String versionId, String orgId) {
		...
		try {
			...
			String hql = " from AfTemplateCollRep mrr where";
	...
	/** 对一张模板设置报送范围 */
	public static boolean  updateHZSD(String childRepId, String versionId, ArrayList orgIds) {
		boolean result = false;
		if (childRepId == null)
			return result;
		if (versionId == null)
		...
		AfTemplateCollRepId comp_id = new AfTemplateCollRepId();
		comp_id.setTemplateId(childRepId);
		comp_id.setVersionId(versionId);

		if (orgIds != null && orgIds.size() > 0) {
			for (int i = 0; i < orgIds.size(); i++) {
				AfTemplateCollRep mrr = new AfTemplateCollRep();
	...
	/** 检查是否有重复记录 */
	public static boolean recordIsExist(AfTemplateCollRepId comp_id) {
		boolean flag = true;
		if (comp_id == null)
			return flag;
		DBConn conn = new DBConn();
		...
		hql.append("from AfTemplateCollRep mrr where mrr.id.orgId='");
	...
	// retVals.add(new LabelValueBean("[" + (String) object[0] + "_" + (String)
	// object[1] + "]" + (String) object[2],(String) object[0] + "-" + (String)
	// object[1]+"-2"));
	// }
	// }
	// } catch (HibernateException he) {
	...
	// }
	/**
	 * 已使用hibernate 卞以刚 2011-12-22 查看该机构是否设置过汇总范围
	 * 
	 * @param orgId
	 * @return
	 */
	public static int getRepRangeCountByOrg(String orgId) {
		...
		try {
			if (orgId != null) {
				...
				Query query = session.createQuery("select count(*) from AfTemplateCollRep mrr where mrr.id.orgId='"
						+ orgId + "'");
				List list = query.list();
				if (list != null && list.size() != 0) {
	...
	public static boolean deleteCollRalationgByOrgId(String orgId) {
		...
		try {
			if (orgId != null) {
				...
				String hql = "from AfTemplateCollRep mrr where mrr.id.orgId='" + orgId + "'";
				session.delete(hql);
				result = true;
			}
		} catch (Exception e) {
	 ...
	 * 
	 * @param reportFlg
	 * @return Administrator 2013-3-21 下午01:52:25 List
	 */
	public static List getPreOrgRepRange(String reportFlg) {
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				for (Iterator iter = list.iterator(); iter.hasNext();) {
					Object[] object = (Object[]) iter.next();
					retVals.add(new LabelValueBean("[" + (String) object[0] + "_" + (String) object[1] + "]"
							+ (String) object[2], (String) object[0] + "-" + (String) object[1] + "-1"));
				}
			}
		} catch (HibernateException he) {
			retVals = null;
