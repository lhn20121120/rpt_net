...
/**
 * Blowfish加密及解密算法，用以接收各业务条线传过来的用户加密解密，该用户直接用用户名称可以登陆统一平台
 * 此算法需要commons-codec-1.6.jar
 * 包支持，用以支持Base64加密解密，用Base64的原因是由于Blowfish加密以后会产生乱码难以用URL传�?
 * 
 * @author Chris
 * 
 */
...
public class Blowfish {
	private final String key = "ke!rong@key#words";
	 ...
	 * @param input
	 *            要加密的字符�?
	 * @return 加密过后的Base64编码字符�?
	 */
	public String encode(final String input) {
		init(true, keyByte);
		return Base64.encodeBase64URLSafeString(encrypt(input));
	}
	...
	public String decode(String source) {
		init(true, keyByte);
		return decryptString(Base64.decodeBase64(source));
	}
