...
/**
 * Displays a MCellFormu. We'll first try to look for a MCellFormu
 ...
 */
public final class ViewMCellFormuAction extends Action {
   ...
   public ActionForward execute(
      ...
      MCellFormuForm MCellFormuForm = (MCellFormuForm)request.getAttribute("MCellFormu");
      if( MCellFormuForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            MCellFormuForm = new MCellFormuForm();
      ...
      // Make a map that can be used to identify this MCellFormu in any links
      HashMap urlParams = new HashMap();
  urlParams.put("cellFormuId",MCellFormuForm.getCellFormuId());
