...
/**
 * Displays a colToFormu. We'll first try to look for a colToFormu
 ...
 */
public final class ViewColToFormuAction extends Action {
   ...
   public ActionForward execute(
      ...
      ColToFormuForm colToFormuForm = (ColToFormuForm)request.getAttribute("colToFormu");
      if( colToFormuForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            colToFormuForm = new ColToFormuForm();
            ...
            request.setAttribute("colToFormu", colToFormuForm);
         } catch (Exception e) {
      ...
      // Make a map that can be used to identify this ColToFormu in any links
      HashMap urlParams = new HashMap();
  urlParams.put("cellFormuId",colToFormuForm.getCellFormuId());
  urlParams.put("childRepId",colToFormuForm.getChildRepId());
  urlParams.put("versionId",colToFormuForm.getVersionId());
  urlParams.put("colName",colToFormuForm.getColName());
