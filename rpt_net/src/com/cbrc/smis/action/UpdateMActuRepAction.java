   ...
   public ActionForward execute(
	  ...
	  try {	  
		  ...
		  if(Config.SYSTEM_SCHEMA_FLAG==1){
			  freqIds = StrutsMActuRepDelegate.findTemplateFreq(mActuRepForm);
		  }
		  ...
		  if(result == false){  //保存失败则返回原页面
			  ...
			  if(Config.SYSTEM_SCHEMA_FLAG==1){
				  if(!"".equals(freqIds)){
					  StrutsMChildReportDelegate.delTaskRelationTemplate(childRepId,freqIds);
				  }
			  }
			  //频率修改成功后，将报表添加到系统管理用户组中 
			  try {
