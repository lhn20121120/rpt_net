...
//                AfTemplate af = AFTemplateDelegate.getTemplate(reportIn.getChildRepId()
//            			,reportIn.getVersionId());
                AfTemplateValiSche valiSche = AfTemplateValiScheDelegate.findAfTemplateValiSche(
                		reportIn.getChildRepId(),reportIn.getVersionId());
                AfReport afRe = AFReportDelegate.getAfReport(reportIn.getChildRepId(),
                		reportIn.getVersionId());
                        ...
                        && dataValidateInfoList.size() > 0)
                {
                    ...
                    if (Report.VALIDATE_TYPE_BN == formuType)
                    {
                    	if(valiSche!=null && valiSche.getValidateSchemeId()!=null &&
                    			valiSche.getValidateSchemeId().equals(1)){
                    		List qtdataValidateInfoList = getTargetDataValidateInfoList(dataValidateInfoList,afRe);
                    		/***
                        	 * jdbc技术 无特殊oracle语法
                        	 * 不需要修改 卞以刚 2011-12-26
                        	 */
                            if (AFDataValidateInfoDelegate.writeDataValidateInfo(orclCon,afRe.getRepId().intValue(),
                            		qtdataValidateInfoList,formuType, reportStyle)) {
                                // 写日志
                            	/***
                ...
                if(valiSche!=null && valiSche.getValidateSchemeId()!=null &&
            			valiSche.getValidateSchemeId().equals(1)){
                	AFReportDelegate.updateFlag(orclCon, afRe.getRepId().intValue(), flagName, validFlag.intValue());
                }
    ...
    public static List getTargetDataValidateInfoList(List sourceDataValidateInfoList,
    					AfReport afRe){
    	List dataValidateInfoList = new ArrayList();
    	for (int i = 0; i < sourceDataValidateInfoList.size(); i++) {
    		DataValidateInfo yjdvi = (DataValidateInfo)sourceDataValidateInfoList.get(i);
    		AfDatavalidateinfo dvi = new AfDatavalidateinfo();
        	AfDatavalidateinfoId dviId = new AfDatavalidateinfoId();
        	
        	dviId.setRepId(afRe.getRepId());
        	String cellFormuValue = AFReportDelegate.getCellFormuValue(yjdvi.getCellFormuId());
        	Long cellFormuId = AFReportDelegate.getCellFormuId(cellFormuValue);
        	dviId.setFormulaId(cellFormuId);
        	
        	dvi.setId(dviId);

            dvi.setValidateFlg(Report.RESULT_NO.longValue());
            dvi.setValidateDesc(yjdvi.getValidateTypeId().toString());
            dvi.setSourceValue(yjdvi.getSourceValue());
            dvi.setTargetValue(yjdvi.getTargetValue());
            
            dataValidateInfoList.add(dvi);
		}
    	return dataValidateInfoList;
    }
