...
/**
 * Displays a MFinaOrg. We'll first try to look for a MFinaOrg
 ...
 */
public final class ViewMFinaOrgAction extends Action {
   ...
   public ActionForward perform(
      ...
      MFinaOrgForm MFinaOrgForm = (MFinaOrgForm)request.getAttribute("MFinaOrg");
      if( MFinaOrgForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            MFinaOrgForm = new MFinaOrgForm();
      ...
      // Make a map that can be used to identify this MFinaOrg in any links
      HashMap urlParams = new HashMap();
  urlParams.put("finaOrgCode",MFinaOrgForm.getFinaOrgCode());
