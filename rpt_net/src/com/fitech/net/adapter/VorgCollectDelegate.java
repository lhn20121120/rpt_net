...
public class VorgCollectDelegate {
	 ...
	 * 更加机构ID，报表年份，月份，查询虚拟机构需汇总报表，报送情况(ok)
	 * */
...
//			pstmt.setString(7, orgId);
	 ...
	 * 获取报表应报机构
	 * */
	public static List getRepOrg(String childrepId,String versionId,Integer dataRgId,Integer year,Integer term,String perOrgId)throws Exception{
		List result=null;
		DBConn conn=null;
		Session session=null;
		Connection con=null;
		...
		try{
			...
			if(ao!=null && ao.getIsCollect().toString().equals("1")){
				...
				sql=" select vmrep.ORG_ID,vmrep.CHILD_REP_ID,vmrep.VERSION_ID," +
						"vmrep.DATA_RANGE_ID,vmrep.REP_FREQ_ID from view_m_report vmrep "+
						...
						"' and vmrep.VERSION_ID='"+versionId+"' and vmrep.DATA_RANGE_ID="+dataRgId;
			}else{
				...
				sql="select vmrep.ORG_ID,vmrep.CHILD_REP_ID,vmrep.VERSION_ID," +
						...
						" where cr.child_rep_id='"+ childrepId + "' and cr.version_id='"+versionId +"')" +
						" and vmrep.VERSION_ID='"+versionId +"' and vmrep.DATA_RANGE_ID="+ dataRgId;
			}
...
//			pstmt.setInt(5, dataRgId.intValue());
			...
			while(rs.next()){
				...
				if(isReped(aditing.getOrgId(), aditing.getChildRepId(), aditing.getVersionId(), aditing.getDataRgId(), year, term))
	 ...
	 * */
	private static boolean isReped(String orgId,String childRepId,String versionId,Integer dataRgId,Integer year,Integer term)throws Exception{
		boolean result=false;
		DBConn conn =null;
		Session session=null;
		Connection con=null;
		...
		ResultSet rs=null;
		try{
			String sql=" select * from report_in rep where rep.org_id=? and rep.child_rep_id=? and rep.version_id=? and rep.year=? and rep.term=? and rep.data_range_id=? and rep.times=1 and rep.check_flag=1";
			conn=new DBConn();
			session=conn.openSession();
			con=session.connection();
			pstmt=con.prepareStatement(sql);
			...
			pstmt.setInt(4, year.intValue());
			pstmt.setInt(5, term.intValue());
			pstmt.setInt(6, dataRgId.intValue());
			rs=pstmt.executeQuery();
			if (rs.next())
				result=true;
		}catch(Exception e){
