		...
		try {
			...
			for (int i = 0; i < list.size(); i++) { // 置数
				...
				if(i%2==0){
					...
					dataset.addValue(value,"计划",label);
					...
					dataset.addValue(_value,"计划完成情况",label);
			...
			if (cs.isC3D())
				...
				chart = ChartFactory.createBarChart3D(graphName, null, tempV,
						dataset, PlotOrientation.VERTICAL, true, false, true); // 得到柱状图对象
...
//				"0.00%")));
				...
				renderer.setItemLabelFont(new Font("宋体", Font.TRUETYPE_FONT, 10));
...
//				"0.00%")));
				...
				renderer.setItemLabelFont(new Font("宋体", Font.TRUETYPE_FONT, 8));
