...
public class ViewNXDataReportAction extends SystemSchemaBaseAction {
...
//			afReportForm.setOrgName(request.getParameter("orgName"));
//			afReportForm.setOrgId(AFOrgDelegate.getIdByName(request.getParameter("orgName")));
//		}
		...
		if(Config.SYSTEM_SCHEMA_FLAG==1 && workTaskTemp!=null && !workTaskTemp.equals("")
				&& !workTaskTemp.equals("null")){
			if("rhtx".equals(workTaskBusiLine)){
				reportFlg = "2";
				afReportForm.setRepFreqId(workTaskFreq);
			}
			...
			afReportForm.setTemplateType(reportFlg);
			if(request.getParameter("workTaskTerm")!=null)
				afReportForm.setDate(request.getParameter("workTaskTerm"));
			if(request.getParameter("workTaskOrgId")!=null)
				afReportForm.setOrgId(request.getParameter("workTaskOrgId"));
			hsqlOld = operator.getChildRepReportPopedom();
			String[] workTaskTemps = workTaskTemp.split(",");
			for(int i=0;i<workTaskTemps.length;i++)
				wtb.append(",'" + afReportForm.getOrgId() + workTaskTemps[i] + "'");
