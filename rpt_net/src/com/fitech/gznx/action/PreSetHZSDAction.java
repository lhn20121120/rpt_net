...
public class PreSetHZSDAction extends SystemSchemaBaseAction {

	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		
		System.out.println("模版汇总设定 action ");
		...
		if (session.getAttribute("SelectedOrgIds") == null) {
			...
			List list = AFCollectRelationDelegate.getAllOrgId();
			if (list != null && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					hMap.put(list.get(i), "ok");
				}
			}
...
//		}
		...
		if(Config.TEMPLATE_MANAGE_FLAG == 1){
			return mapping.findForward("nextTo");
		}else{
