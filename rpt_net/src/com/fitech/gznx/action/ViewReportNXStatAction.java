...
import com.fitech.gznx.po.ViewWorkTaskTemplate;
...
public class ViewReportNXStatAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request
		...
		if(Config.SYSTEM_SCHEMA_FLAG == 1){
			List list = AFReportDelegate.getTaskInfo(reportFlg);
			request.setAttribute("taskInfo", list);
		}
		/***************任务模式(end)**********************/
...
//		}	
		...
		/**加入报表强制上报查询条件*/
		if(reportInForm.getIsFlag() != null){
			term +=(term.indexOf("?")>=0 ? "&" : "?");
			term +="isFlag="+reportInForm.getIsFlag();
		}
		/**加入任务查询条件*/
		if(reportInForm.getTaskId() != null){
			term +=(term.indexOf("?")>=0 ? "&" : "?");
			term +="temp="+reportInForm.getTaskId();
		}
		if(term.indexOf("?")>=0)		
