		...
		try {
			...
			if (isMulti != null && isMulti.equals("multi")) {//批量汇总
				...
				for (int i = 0; i < parArr.length; i++) {//多张循环
					...
					messages.add("");
					if(rs)
						messages.add(org.getOrgName() +  strArr[0] + "数据汇总成功！<br/>");
					else
						messages.add(org.getOrgName() +  strArr[0] + "数据汇总未完成！<br/><br/>");
...
//				}
				...
				if (reportInID == null)
					messages.add(childRepId + "报表汇总失败（汇总数据异常）！<br/>");
				else if (reportInID.intValue() == -1)
					messages.add(childRepId + "报表汇总失败（未设置汇总关系）！<br/>");
				else
					messages.add(childRepId + "报表汇总成功！<br/>");
			}
			if (messages.getMessages() != null
					&& messages.getMessages().size() > 0)
				request.setAttribute(Config.MESSAGES, messages);
