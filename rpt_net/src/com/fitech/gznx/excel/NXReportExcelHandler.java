...
public class NXReportExcelHandler {
	 ...
	 * @param reptId
	 */
	public NXReportExcelHandler(Integer repInId, String excelFilePath,FitechMessages messager)
			throws Exception {
		messages = messager;
				...
				if (repType.equals(Config.PBOC_REPORT)){
					...
					for (int i = 0; i < cellList.size(); i++) {
						...
						if(com.cbrc.smis.common.Config.ISADDTRACE){
							...
							try {
								finalData = Double.valueOf(cell.getCellValue())+sumData;
								insertSQL = "insert into af_otherreportdata (Rep_ID,Cell_ID,Cell_Data) values("
										+ cell.getRepInId()
										+ ","
										+ cell.getCellId()
										+ ",'"
	...
	private List getDataFromExcels(String repInId) throws Exception {
		...
		try {
				...
				if (cell.getCellName() != null) {
					...
					try {
						excelcell = sheet.getRow(row).getCell(col);
		 				...
		 				if("".equals(cellValue) && isMergedRegion(sheet,row,col)){//解决读取excel时存在合并单元格的问题，会出现取不到合并第一个后面单元格的值,这里改为只读取合并后第一个单元格的值;
		 					continue;
		 				}
			...
			result = null;
			throw new Exception ("读取报表文件出现错误!");
		} finally {
			if (inStream != null)
				inStream.close();
		}
