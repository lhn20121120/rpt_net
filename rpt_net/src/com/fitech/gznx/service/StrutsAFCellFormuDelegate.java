...
import com.fitech.fomula.service.DBFomulaDelegate;
...
public class StrutsAFCellFormuDelegate  extends DaoModel{
	 ...
	 * @return boolean 导入成功，返回true;否则，返回false
	 */
	public static boolean saveYJPatch(ReportCheckForm reportCheckForm ){
		...
		try{
			conn=new DBConn();
			session=conn.beginTransaction();
			if(reportCheckForm!=null && reportCheckForm.getValidateTypeId()!=null){
				String hql ="select max(m.cellFormuId) from MCellFormu m";
				Integer cellFormuId = (Integer)session.createQuery(hql).uniqueResult();
				if(cellFormuId==null)
					cellFormuId=1;
				MCellFormu mCellformu=new MCellFormu();
				mCellformu.setCellFormu(reportCheckForm.getFormulaValue());
				mCellformu.setCellFormuId(cellFormuId+1);
				mCellformu.setCellFormuView(reportCheckForm.getFormulaName());
				mCellformu.setFormuType(Integer.valueOf(reportCheckForm.getValidateTypeId()));
				session.save(mCellformu);
				
				hql ="select max(m.cellToFormuId) from MCellToFormu m";
				Integer cellToFormuId = (Integer)session.createQuery(hql).uniqueResult();
				if(cellToFormuId==null)
					cellToFormuId=1;
				MCellToFormu mCellToFormu = new MCellToFormu();
				mCellToFormu.setCellFormuId(mCellformu.getCellFormuId());
				mCellToFormu.setCellToFormuId(cellToFormuId+1);
				mCellToFormu.setChildRepId(reportCheckForm.getTemplateId());
				mCellToFormu.setVersionId(reportCheckForm.getVersionId());
				session.save(mCellToFormu);
				result=true;
			}
		}catch(Exception e){
			log.printStackTrace(e);
...
//			session.flush();
	...
	public static List<MCellToFormu> selectALLCellFormuByTemplate(String childRepId,String versionId){
		DBConn conn = null;
    	Session session = null;
    	//select * from m_cell_formu m left join m_cell_to_formu t on m.cell_formu_id=t.cell_formu_id where t.child_rep_id='G0100' and t.version_id='1010'
    	List<MCellToFormu> list = new ArrayList<MCellToFormu>();
    	try{    		
			conn = new DBConn();
			session = conn.beginTransaction();
    		
			String hql="from MCellToFormu t where t.childRepId='"+childRepId+"' and t.versionId="+versionId;
			list= conn.openSession().find(hql);			
	...
	public static boolean delete(String cellFormuIds,String templateId,String versionId, String reportFlg){
		...
		try{
			...
			if(reportFlg.equals(com.fitech.gznx.common.Config.CBRC_REPORT)){
				...
				try {
					DBFomulaDelegate.delAfPreFormula("2", cellFormuIds);
				} catch (Exception e) {
