...
function _back(childRepId, versionId, upOrgId) {
	...
	window.location = "<%=request.getContextPath()%>/template/mod/preEditBSSD.do?childRepId="
...
<body>
	...
	<html:form action="/template/mod/updateBSFW.do" method="post"
		styleId="frmSel">
		...
		<TABLE cellSpacing="1" cellPadding="4" width="96%" border="0"
			align="center" class="tbcolor">
			<TR class="tbcolor1">
				<th align="center" id="list" height="30" colspan="2"><span
					style="FONT-SIZE: 11pt" > <logic:present name="ReportName"
							scope="request">
							《<span id="hidden_reportName"><bean:write name='ReportName' /></span>》
						</logic:present>报送设定 </span></th>
						
			</TR>
			<tr>
				<td>级联选择： <input type="checkbox" name="checkbox_jl_org"
					onclick="return checkbox_jl()"></td>
			</tr>
			<tr>
			...
			<logic:present name="isNull" scope="request">
				<tr colspan="8">
					<td colspan="8" align="center">暂无机构信息</td>
				</tr>
				<tr>
					<td align="center"><input type="button" value="返  回"
						class="input-button" onclick="goback();" /></td>
				</tr>
			</logic:present>
			<logic:notPresent name="isNull" scope="request">
				...
				<tr>
					<td align="center"><input type="button" value="下一步"
						class="input-button" onclick="getAllCheckedId()"><%-- &nbsp; <input
						type="button" value="返  回" class="input-button"
						onclick="goback();" /></td>--%>
					<td></td>
...
function getAllCheckedId() {
	var reportName=document.getElementById("hidden_reportName").innerHTML;
	var checkId = tree2.getAllChecked();
	if (checkId.replace(/(^[\s]*)|([\s]*$)/g, "") == '') {
		alert("请选择报表上报机构!");
		return;
	} else {//gongming 2008-07-26
		window.location = "<%=request.getContextPath()%>/gznx/updateBSSD.do?orgIds=" + checkId
				+ "&childRepId=<%=childRepId%>&versionId=<%=versionId%>&ReportName="+reportName+"&opration=next";
	}
}
