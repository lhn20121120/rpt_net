...
String reportName = "";
  ...
  Operator operator=(Operator)session.getAttribute(Config.OPERATOR_SESSION_ATTRIBUTE_NAME);
	...
	/*add 姜明青 判断是否是任务模式*/
	int templatemanageflag = 0;
	if(request.getAttribute("template_manage_flag")!=null){
		templatemanageflag=(Integer)request.getAttribute("template_manage_flag");
	}
	System.out.println(templatemanageflag);
%>
<html:html locale="true">
	...
	<script language="javascript">
		...
		function viewAFTemplate(orgId,childRepId,versionId,reportName){
			window.location="<%=request.getContextPath()%>/template/jiaoYanAction.do" + 
			  		"?childRepId=" + childRepId + 
			 		"&versionId=" + versionId + 
			 		"&reportName="+reportName+
			 		"&orgId="+orgId+"&type=edit";
		}
				...
				<TR>
					<TD>
						<TABLE cellSpacing="1" cellPadding="4" width="100%" border="0"  class="tbcolor">
							<tr class="titletab">
								<th colspan="11" align="center" id="list">
									<strong>										
						...
						<logic:iterate id="item" name="Records">
							<TR bgcolor="#FFFFFF">
								...
								<TD  align="center" width="10%">
									<a href="javascript:viewAFTemplate('<%=orgId%>','<bean:write name="item" property="templateId"/>','<bean:write name="item" property="versionId"/>','<bean:write name="item" property="templateName"/>')">修改</a>
								</TD>
								<% if(templatemanageflag ==0){ %>
								<TD  align="center" width="10%">
									<a href="javascript:viewHZSD('<%=orgId%>','<bean:write name="item" property="templateId"/>','<bean:write name="item" property="versionId"/>','<bean:write name="item" property="templateName"/>')">修改</a>
								</TD>
								<%} %>
								<%if(reportFlg.equals("2")){ %>
								<TD  align="center" width="10%">
									
									<a href="javascript:viewHZGS('<%=orgId%>','<bean:write name="item" property="templateId"/>','<bean:write name="item" property="versionId"/>','<bean:write name="item" property="templateName"/>')">修改</a>
								</TD>
								<%} %>
								<TD  align="center" width="10%">
									
									<a href="javascript:viewBSSD('<%=orgId%>','<bean:write name="item" property="templateId"/>','<bean:write name="item" property="versionId"/>','<%=reportName %>')">修改</a>
								</TD>
								<TD  align="center" width="10%">
