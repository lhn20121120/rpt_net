			...
			if(repInIds==""){
				...
				if(flag==FLAG_OK){
					...
					if(confirm("您确定复核不通过吗?\n")==true){
						window.location="<%=request.getContextPath()%>/gznx/report_data/report_again_add_nx.jsp?" + qry+"&updateFlag=1";			  	  				
					}
				}
			}
			...
			try{
				...
				if(<%=Config.SYSTEM_SCHEMA_FLAG%>==0){
					...
					if(result == 'true'){
						 ...
						 if(confirm('校验未通过！是否需要查看校验信息?')){
					        window.open("<%=request.getContextPath()%>/report/viewMoreValidateInfo.do?" + "repInIds=" + result,'校验结果','scrollbars=yes,height=600,width=600,status=yes,toolbar=no,menubar=no,location=no,resizable=yes');  
					      }
					}
				}
				selectReport();
...
<body onload="isHiddenQueryTable()">
			...
			<html:form action="/dateQuary/saveAllCheckRepNXRecheck" method="post" styleId="frm1">
				<tr>
					<td>
						...
						<input type="button" value="校验选中" disabled="disabled" class="input-button" onclick="return _batchVal()">
						<%
							if(Config.SYSTEM_SCHEMA_FLAG==0){
							%>
							&nbsp;
						<input type="button" value="校验全部" disabled="disabled" class="input-button" onclick="return _batchAllVal()">
							<%
							}
						 %>
						&nbsp;&nbsp;∷&nbsp;&nbsp;
						<input type="button" value="审核通过" disabled="disabled" class="input-button" onclick="_batchCheck(FLAG_OK)">
						&nbsp;
						<%if(Config.SYSTEM_SCHEMA_FLAG==0){
							%>
							<input type="button" value="全部通过" disabled="disabled" class="input-button" onclick="return _batchAllCheck(FLAG_OK)">
							&nbsp;&nbsp;∷&nbsp;&nbsp;
							<%
						} %>
						<input type="button" value="审核不通过" disabled="disabled" class="input-button" onclick="_batchCheck(FLAG_NO)">
						<%}else{ %>
							...
							<logic:iterate id="viewReportIn" name="form">
								...
								<TR bgcolor="#FFFFFF"  onmouseover="changeToLavender(this)" onmouseout="changeToWhite(this)">
									<td align="center">	
										...
										<input type="hidden" name="sync<%=i%>" id="sync<%=i%>" value="<bean:write name="viewReportIn" property="childRepId"/>_<bean:write name="viewReportIn" property="versionId"/>"/>
									</td>
									<TD align="center">
									...
									<TD align="center">
											<logic:present name="viewReportIn" property="isFlag">
												<logic:equal name="viewReportIn" property="isFlag" value = "1">
													是
												</logic:equal>
												<logic:equal name="viewReportIn" property="isFlag" value = "0">
													否
												</logic:equal>
											</logic:present>
									</TD>
	...
	<%--<button onclick="buURL()" id="buURL" >${url }</button>
	<script type="text/javascript">
	var url = document.getElementById("buURL").innerHTML;
	function buURL(){
		...
		//window.location = urlPath +"?"+parameters;
		
		var paras = parameters.split("&");
		$("#hiddenForm").attr("action",urlPath);
		for(i=0;i<paras.length;i++){
			var docuName = paras[i].split("=")[0];
			var value = paras[i].split("=")[1];
			//alert(docuName+"="+value)
			var newInput = $("<input name=\""+docuName+"\" value=\""+value+"\">");
			newInput.appendTo($("#hiddenForm"));
			newInput.after($("#hiddenIP"));
		}
		alert(document.getElementById("hiddenForm").innerHTML)
		$("#hiddenForm").submit();
	*/
	}
	</script>
