  ...
  <head>
	...
	<script type="text/javascript">
		...
		function showHenji(){
				...
				if(dataArray[i]==oriId){//判断是否被修改过
					//for(j=dataArray[oriId].length-1;j>=0;j--){
						...
						beizhu +="<tr id='changeTr'><td>"+henji.userName+"</td><td>"
								  +henji.changeTime+"</td><td id='firstTD'>"
								  +henji.oriData+"</td><td><input  id='changeInput' style='width:100%' value='"
			...
			if(!hava){
				//没有被修改
				beizhu += "<tr id='changeTr' onmouseover=\"this.style.backgroundColor='#EEEEFF'\" onmouseout=\"this.style.backgroundColor='white'\" ><td>admin</td><td>当前时间</td><td id='firstTD'>"
							+obj.val()+"</td><td><input  id='changeInput' style='width:100%'/></td><td><input id='finalInput' style='width:100%'/></td><td><input type='text' id='descInput' style='width:100%' /></td><td><span id='changeSpan' style='visibility: hidden'>&nbsp;<img src='<%=request.getContextPath() %>/image/check_right.gif' alt='保存修改' style='cursor:hand' onclick='saveData()'/></span></td></tr>";
				oriNum = obj.text();//原始值
			}
			<%}%>
			...
			$.post(url,function(data){
				...
				$("#changeInput").bind("keyup",function(){
					...
					if($(this).val()==""){
						...
						//最终值
						$("#finalInput").val(parseFloat($("#firstTD").text())+parseFloat($(this).val()));
				...
				$("#finalInput").bind("keydown",function(){
					...
					if(!(event.keyCode==190 || event.keyCode==110 
						...
						event.cancelBubble = true;//取消时间冒泡
						return false;
					}
				});
				//调整结果输入绑定事件改变调整值
				$("#finalInput").bind("keyup",function(){
					...
					if($(this).val()==""){
						...
						//调整值
						$("#changeInput").val(parseFloat($(this).val())-parseFloat($("#firstTD").text()));
		...
		function saveSjhj(){
			...
			if(!first)
				param+="&&templateId=<%=templateId%>&&versionId=<%=versionId%>";
			//alert(param);
		...
		<table width="100%">
			<tr align="center">
				<td>
					...
					&nbsp;&nbsp;&nbsp;
					<input type="button" class="input-button" value=" 反查公式查看   "onclick="window.open('sjfc.jsp')">
					&nbsp;&nbsp;&nbsp;
