		...
		<!--
			 ...
			 */
			 function _submitFrm(){	 			
			   ...
			   var formObj=document.getElementById("frm1");
				...
				var ids=tree2.getAllChecked();
				if(isEmpty(ids)==true){
					alert("请选择机构项!");
					return false;
				}
				form.orgCls.value=tree2.getAllChecked();
			 ...
			 */
			  ...
			  */
			   ...
			   */ 
			   function goBack(){
					window.location="<%=request.getContextPath()%>/template/viewTemplateDetail.do?childRepId=<%=childRepId%>&reportName=<bean:write name="ObjForm" property="reportName"/>";
			   }	
			   /**
			    *进入机构设置页面
			    */
...
<body background="../image/total.gif">
		...
		<tr>
			...
			<form method="post"  name="frm1" action="../ycbh/updateYCBH.do" >
			 ...
			 <logic:notPresent name="isNull" scope="request">
				 <tr>
				<td valign="top">
					<div id="treeboxbox_tree2" style="width:100%; height:200;background-color:#f5f5f5;border :1px solid Silver;; overflow:auto;"></div>
				</td>		
			 </logic:notPresent>			
				</table>
				</form>
	...
	<form method="post"  name="frm" action="../mod/updateYCBH.do" onsubmit="return _submitFrm()">
	...
	//			alert(checkId);
	//			window.location="<%=request.getContextPath()%>/template/ycbh/UpdataYCBHAction.do?orgIds="+checkId+"&childRepId="+'<%=childRepId%>'+"&versionId="+'<%=versionId%>';
	//}		
