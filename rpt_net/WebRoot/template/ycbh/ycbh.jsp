		...
		<!--
			 ...
			 */
			 function _submitFrm(){
			 		...
			 		var _orgCls="";       //选择的机构类型串
					
					if(is_set_org_or_orgcl=="0"){
				 		if(typeof(objCount)!="undefined"){
				 			var _count=objCount.value;
	
				 			for(var i=0;i<_count;i++){
				 				var objChb=eval("document.getElementById(\"chb" + i + "\")");
				 				if(objChb.checked==true) _orgCls+=(_orgCls==""?"":SPLIT_SMYBOL_COMMA) + 
				 				"'" + objChb.value + "'";
				 			}
				 		}
	
				 		if(isEmpty(_orgCls)==true){
				 			alert("请设定异常变化所适用的机构类型!\n");
				 			return false;	
				 		}
			 		}
			 		
			 		form.orgCls.value=_orgCls;
			  ...
			  */
			   ...
			   */ 
			   function _back(){
	  		 	 ...
	  		 	 window.location="<%=request.getContextPath()%>/template/ycbh/viewMChildReport.do?" +
...
<body background="../image/total.gif">
							...
							<tr>
								...
								<td align="left">
									...
									<logic:equal name="item" property="selAll" value="0">
										...
										<logic:equal name="item" property="selected" value="1"> checked</logic:equal>
										>
										<a href="javascript:_set_detail('<bean:write name="item" property="orgClsId"/>','<bean:write name="item" property="orgClsNm"/>')">
											<bean:write name="item" property="orgClsNm" />
										</a>
	...
	<form name="frm" action="updateYCBH.do" onsubmit="return _submitFrm()">
