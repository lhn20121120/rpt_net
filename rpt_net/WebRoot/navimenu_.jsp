	...
	<table border="0" cellpadding="0" cellspacing="1">
		...
		<tr>
			<td height="30" width="160" align="center">
				<select id="reportFlg" style="background-color:#ebebeb;FONT-SIZE: 10.5pt" onchange="changeReport(this)">
					<%
						if (reportFlg.equals("0")) {
					%>
					<option value="0" style="background-color:#ebebeb;FONT-SIZE: 10.5pt"  selected>报表类型选择</option>
					<%
						}else{
					 %>
					<option value="0"  style="background-color:#ebebeb;FONT-SIZE: 10.5pt" >返回首页</option>
					<% 
						}if(yjhbb){
						if(reportFlg.equals("1")) {
					%>
					<option value="1" style="background-color:#ebebeb;FONT-SIZE: 10.5pt"  selected>银监会报表</option>
					<% 
						} else {						
					%>
					<option value="1" style="background-color:#ebebeb;FONT-SIZE: 10.5pt" >银监会报表</option>
					<%
					 	}}
					 	if(rhbb){
					 	if(reportFlg.equals("2")) {
					%>
					<option value="2" style="background-color:#ebebeb;FONT-SIZE: 10.5pt"  selected>人行报表</option>
					<%
						}else{						
					 %>
					 <option value="2" style="background-color:#ebebeb;FONT-SIZE: 10pt" >人行报表</option>
					 <% 
					 	} }
					 	if(qtbb){
					 	if(reportFlg.equals("3")) {
					 %>
					<option value="3" style="background-color:#ebebeb;FONT-SIZE: 10.5pt"  selected>其他报表</option>
					<%
						}else{						
					 %>
					 <option value="3" style="background-color:#ebebeb;FONT-SIZE: 10.5pt" >其他报表</option>
					 <%}} %>
				</select>
			</td>
		</tr>
