...
{ 
	...
	{ 
		sDate = document.all.Year.value + "-" + 
				(document.all.Month.value<10?"0"+document.all.Month.value:document.all.Month.value) + "-" + 
				(event.srcElement.innerText<10?"0"+event.srcElement.innerText:event.srcElement.innerText);
				//document.all.Month.value+"-"+event.srcElement.innerText;
...
function DeleteCalendar()
{
	...
	//document.getElementsByTagName("date").value='';
}
...
function showCalendar(InputBox)
{
	...
	DivContent="<table border='0' cellspacing='0' style='width:200px;border:1px solid #0066FF; background-color:#EDF2FC'>";
	DivContent+="<tr>";
	DivContent+="<td style='border-bottom:1px solid #0066FF; background-color:#C7D8FA'>";
	
	//年
	DivContent+="<select name='Year' id='Year' onChange='newCalendar()' style='font-family:Verdana; font-size:12px'>";
	for (intLoop = thisyear - 15; intLoop < (thisyear + 20); intLoop++) 
	...
	DivContent+="<td style='border-bottom:1px solid #0066FF; width:38px;background-color:#C7D8FA;font-size:12px;color:#4477FF; cursor:hand' title='清空' onClick='javascript:DeleteCalendar()'>清空</td>";
