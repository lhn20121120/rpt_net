...
<%
	...
	//开启任务模式
	Integer schemaflag = Config.SYSTEM_SCHEMA_FLAG;
%>
...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function saveSjhj(){
			...
			for(i=0;i<dataArray.length;i++){
				if(dataArray[i]!=null && dataArray[i]!="")
				{
					...
					//		+(henji.desc==null||henji.desc==""?"":"&&desc="+henji.desc);
					 // 创建一个输入
				    var input_cellName = document.createElement("input");
				    // 设置相应参数
				    input_cellName.type = "hidden";
				    input_cellName.name = "cellName";
				    input_cellName.value = (henji.cellName==""||henji.cellName==null?"''":henji.cellName);

				    // 将该输入框插入到 form 中
				    form1.appendChild(input_cellName);
				    
				    var input_originalData=document.createElement("input");
				    input_originalData.type="hidden";
				    input_originalData.name="originalData";
				    input_originalData.value=(henji.oriData==""||henji.oriData==null?"''":henji.oriData);
				    // 将该输入框插入到 form 中
				    form1.appendChild(input_originalData);
				    
				    var input_changeData=document.createElement("input");
				    input_changeData.type="hidden";
				    input_changeData.name="changeData";
				    input_changeData.value=(henji.changeData==""||henji.changeData==null?"''":henji.changeData);
				    // 将该输入框插入到 form 中
				    form1.appendChild(input_changeData);

				    var input_finalData=document.createElement("input");
				    input_finalData.type="hidden";
				    input_finalData.name="finalData";
				    input_finalData.value=(henji.finalData==""||henji.finalData==null?"''":henji.finalData);
				    // 将该输入框插入到 form 中
				    form1.appendChild(input_finalData);
				    
				    var input_desc=document.createElement("input");
				    input_desc.type="hidden";
				    input_desc.name="desc";
				    input_desc.value=(henji.desc==null||henji.desc==""?"":henji.desc);
				    // 将该输入框插入到 form 中
				    form1.appendChild(input_desc);
			...
			//new Ajax.Request(url,{method: 'post',parameters:param,onComplete:"",onFailure: ""});
			 var input_repInId=document.createElement("input");
				    input_repInId.type="hidden";
				    input_repInId.name="repInId";
				    input_repInId.value="<%=repInId%>";
				    // 将该输入框插入到 form 中
				    form1.appendChild(input_repInId);
			 // form 的提交方式
		    form1.method = "POST";
		    // form 提交路径
		    form1.action = "<%=request.getContextPath()%>/addAFDataTrace.do";
		...
		function getNoValidateFormu(){
			var formu="";
            $.ajax({  
                type: "post",  
                url: "<%=request.getContextPath()%>/report/getNoValidateFormu.do?repInId=<%=repId%>",  
	...
	function  addNoValidTdColor(){
		var msg = getNoValidateFormu();
		if(msg!= null &&msg!=""){
			var msgs = msg.split("##");
			 if(scriptReportFlg=="1"){
				for (var i=0;i<msgs.length;i++){
					var tdStr = msgs[i].split("=")[0];
					var td = tdStr.substring (tdStr.indexOf("[")+1,tdStr.indexOf("]"))
					changeColor(td);
				}
			 }else{
				 for (var i=0;i<msgs.length;i++){
		...
		<table width="100%">
			<tr align="center">
				<td>
					...
					<%}else{%>
						<input type="button" disabled class="input-button" value=" 导出EXCEL" onclick="report1_saveAsExcel1('${requestScope.backQry}',${requestScope.repInId})">
					<% } %>	
					&nbsp;&nbsp;&nbsp;
					<%if(schemaflag == 0){ %>
