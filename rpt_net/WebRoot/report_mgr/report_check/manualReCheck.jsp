	...
	<SCRIPT language="javascript">
		...
		function _batchCheck(flag){
			...
			if(repInIds==""){
				...
				if(flag==FLAG_OK){
					if(confirm("您确定要做当前的复核通过操作吗?\n")==true){
						window.location="<%=request.getContextPath()%>/dateQuary/saveCheckRepRecheck.do?" + qry;			  	  				
					}
				}else{
					if(confirm("您确定要做当前的复核不通过操作吗?\n")==true){
						window.location="<%=request.getContextPath()%>/report_mgr/report_check/report_again_add_re.jsp?" + qry;			  	  				
					}
				}
			}
		}
...
<body>
	...
	<table cellspacing="0" cellpadding="4" border="0" width="98%" align="center">
		<html:form action="/dateQuary/manualCheckRepRecheck" method="post" styleId="frm" onsubmit="return _submit(this)">
	...
	<logic:present name="form" scope="request">
		<table cellSpacing="0" cellPadding="4" width="98%" border="0" align="center">
			<html:form action="/dateQuary/saveAllCheckRepRecheck" method="post" styleId="frm1">
				<tr>
					<td>
						<input type="button" value="复核通过" class="input-button" onclick="_batchCheck(FLAG_OK)">
						&nbsp;
						<input type="button" value="复核不通过" class="input-button" onclick="_batchCheck(FLAG_NO)">
						&nbsp;
					<%-- 	<input type="button" value="全部报表复核通过" class="input-button" onclick="return _batchAllCheck(FLAG_OK)"> --%>
					</td>
				</tr>				
			</html:form>
		</table>
		...
		<TR>
			<TD>
				<jsp:include page="../../apartpage.jsp" flush="true">
					<jsp:param name="url" value="../../dateQuary/manualCheckRepRecheck.do" />
				</jsp:include>
			</TD>
		</TR>
	</table>
