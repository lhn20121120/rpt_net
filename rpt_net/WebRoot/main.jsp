...
<html:html locale="true">
	...
	<%
		String reportPortalFlag = null;
		if(session.getAttribute("reportPortalFlag")!=null){
			reportPortalFlag = (String)session.getAttribute("reportPortalFlag");
		}
			...
			<script type="text/javascript">
				try{	
					$(function(){
						$.ajax({   
						          type : "post",   
						          url : "/rpt_other/loginSynchronAction.action",   
						          data : encodeURI("op.roleIds=<%=operator.getRoleIds()%>"+
									"&op.operatorId=<%=operator.getOperatorId()%>"+
									"&op.operatorName=<%=operator.getOperatorName()%>"+
									"&op.orgId=<%=operator.getOrgId()%>"+
									"&op.orgName=<%=operator.getOrgName()%>"+
									"&op.business=<%=reportPortalFlag%>"+
									"&op.userName=<%=operator.getUserName()%>"),
						          async : false,   
						          success : function(data){   
										if(data==undefined || parseInt(data)==1){
											alert("rpt_other系统故障....");
										}else{
											location.href='/rpt_other/pendingTaskAction!initSearch.action';
										}
						          }   
					          });
					})
